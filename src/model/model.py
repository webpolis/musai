...
if 'x070' in os.environ["RWKV_MY_TESTING"]:
    ...
    load(name="wind_backstepping", sources=[
         f'{CUDA_SRC_PATH}/wkv7_cuda.cu', f'{CUDA_SRC_PATH}/wkv7_op.cpp'], is_python_module=False, verbose=True, extra_cuda_cflags=flags)
        ...
        def backward(ctx, gy):
            with torch.no_grad():
                ...
                gv = torch.empty((B, T, C), device=gy.device, requires_grad=False,
                                 # .uniform_(-1, 1)
                                 dtype=torch.bfloat16, memory_format=torch.contiguous_format)
                gw = torch.empty((B, C), device=gy.device, requires_grad=False, dtype=torch.bfloat16,
                                 # .uniform_(-1, 1)
                                 memory_format=torch.contiguous_format)
                gu = torch.empty((B, C), device=gy.device, requires_grad=False, dtype=torch.bfloat16,
                                 # .uniform_(-1, 1)
                                 memory_format=torch.contiguous_format)
                wkv5_cuda.backward(B, T, C, H, r, k, v, eew,
...
class RWKV_Tmix_x052(MyModule):
    def __init__(self, args, layer_id):
        ...
        with torch.no_grad():
            ...
            for n in range(args.dim_att):
                decay_speed[n] = -6 + 5 * \
                    (n / (args.dim_att - 1)) ** (0.7 + 1.3 * ratio_0_to_1)
    ...
    def forward(self, x):
        ...
        x = RUN_CUDA_RWKV5(B, T, C, H, r, k, v,
                           w=self.time_decay, u=self.time_faaaa)
    ...
    def __init__(self, args, layer_id):
        ...
        with torch.no_grad():
            ...
            self.time_maa_w1 = nn.Parameter(
                torch.zeros(args.n_embd, D_MIX_LORA*5))
            self.time_maa_w2 = nn.Parameter(torch.zeros(
                5, D_MIX_LORA, args.n_embd).uniform_(-0.01, 0.01))
            ...
            for n in range(args.dim_att):
                decay_speed[n] = -6 + 5 * \
                    (n / (args.dim_att - 1)) ** (0.7 + 1.3 * ratio_0_to_1)
            self.time_decay = nn.Parameter(
                decay_speed.reshape(1, 1, args.dim_att))

            D_DECAY_LORA = 64
            self.time_decay_w1 = nn.Parameter(
                torch.zeros(args.n_embd, D_DECAY_LORA))
            self.time_decay_w2 = nn.Parameter(torch.zeros(
                D_DECAY_LORA, args.dim_att).uniform_(-0.01, 0.01))
        ...
        self.output = nn.Linear(args.dim_att, args.n_embd, bias=False)
        self.gate = nn.Linear(args.n_embd, args.dim_att, bias=False)
        self.ln_x = nn.GroupNorm(self.n_head, args.dim_att, eps=(
...
class RWKV_Tmix_x060_state(MyModule):
    def __init__(self, args, layer_id):
        ...
        with torch.no_grad():
            ...
            self.time_faaaa = nn.Parameter(
                tmp.reshape(self.n_head, self.head_size))
            self.time_state = nn.Parameter(torch.zeros(
...
class RWKV_Tmix_x060a(MyModule):
    def __init__(self, args, layer_id):
        ...
        with torch.no_grad():
            ...
            D_GATE_LORA = 64
            self.gate_w1 = nn.Parameter(torch.empty(
                args.n_embd, D_GATE_LORA).uniform_(-0.01, 0.01))
            self.gate_w2 = nn.Parameter(torch.zeros(
                D_GATE_LORA, args.n_embd).uniform_(-0.01, 0.01))
...
class RWKV_Tmix_x060b(MyModule):
    def __init__(self, args, layer_id):
        ...
        with torch.no_grad():
            ...
            self.time_maa_w = nn.Parameter(
                1.0 - torch.pow(ddd, ratio_1_to_almost0))
            D_MIX_LORA = 32
            self.time_maa_rkvw_w1 = nn.Parameter(
                torch.zeros(args.n_embd, D_MIX_LORA*4))
            self.time_maa_rkvw_w2 = nn.Parameter(torch.zeros(
                4, D_MIX_LORA, args.n_embd).uniform_(-0.01, 0.01))
    ...
    def forward(self, x):
        ...
        xxx = x + xx * self.time_maa_x
        xxx = torch.tanh(xxx @ self.time_maa_rkvw_w1).view(B *
                                                           T, 4, -1).transpose(0, 1)
        ...
        v = self.value(v)
        w = self.time_decay + \
            torch.tanh(w @ self.time_decay_w1) @ self.time_decay_w2
...
class RWKV_Tmix_x060c(MyModule):
    ...
    def forward(self, x):
        ...
        k = k * (1-(-w.float().exp()).exp()
                 ).to(dtype=torch.bfloat16)  # for bf16
...
class RWKV_Tmix_x070(MyModule):
    def __init__(self, args, layer_id):
        ...
        self.args = args
        self.layer_id = layer_id
        self.my_testing = os.environ["RWKV_MY_TESTING"] 
        ...
        with torch.no_grad():
            ...
            for n in range(C):
                decay_speed[n] = -7 + 5 * \
                    (n / (C - 1)) ** (0.85 + 1.0 * ratio_0_to_1 ** 0.5)
            ...
            # !!! notice eps value !!!
            self.ln_x = nn.GroupNorm(H, C, eps=(
                1e-5)*(args.head_size_divisor**2))
    ...
    def __init__(self, args, layer_id):
        ...
        with torch.no_grad():  # fancy init of time_mix
            ...
            for i in range(args.n_embd):
                ddd[0, 0, i] = i / args.n_embd
            self.time_maa_k = nn.Parameter(
                1.0 - torch.pow(ddd, ratio_1_to_almost0**3))
            self.time_maa_r = nn.Parameter(
                1.0 - torch.pow(ddd, ratio_1_to_almost0**3))
    ...
    def __init__(self, args, layer_id):
        ...
        if self.layer_id == 0 and self.args.pre_ffn > 0:
            ...
            if 'x070' in os.environ["RWKV_MY_TESTING"]:
                ...
                self.att = Mamba(d_model=args.n_embd, d_state=16,
                                 d_conv=4, expand=2.125)  # match rwkv6 #params
        ...
        if 'x070' in os.environ["RWKV_MY_TESTING"]:
            ...
            self.ffn = Mamba(d_model=args.n_embd, d_state=16,
                             d_conv=4, expand=2.125)  # match rwkv6 #params
...
class RWKV(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if args.layerwise_lr > 0:
            ...
            optim_groups = [{"params": [param_dict[n]
                                        for n in lr_1x], "weight_decay": 0.0, "my_lr_scale": 1.0}]
...
"""
        ...
        for n in self.state_dict():
            ...
            print(f"{s0.ljust(5)} {s1.ljust(5)} {
                  s2.ljust(5)} {s3.ljust(5)} {n}", end="")
